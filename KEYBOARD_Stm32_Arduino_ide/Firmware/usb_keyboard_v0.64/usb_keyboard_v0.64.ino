//Changes--->cap_led,next and previous, volume change linearly,mouse right click
...
int normal[rows_max][cols_max] = {
  ...
  {0,'z','a','q','1',KEY_TAB,BCKTCK,KEY_ESC},
...
void loop() {
   boolean shiftPressed = mod_shift_l || mod_shift_r; // Check if any Shift key is pressed
  ...
  for (int x=0; x<rows_max; x++)  
  {
    ...
    for (int y = 0; y < cols_max; y++) 
    {
            ...
            if (modifier[x][y] != 0)     //Means there is modifier key present in that position
            {
                    ...
                    if (!Fn_pressed)
                    {
                          if (modifier[x][y] == KEY_RIGHT_CTRL) {
                                      RightCtrlPressed = !digitalRead(Col_IO[y]);
                          }
                          if (RightCtrlPressed) 
                          {
                            // Debounce delay to avoid rapid triggering
                            delay(50);
                              ...
                              // Set the flag to indicate that the combination was pressed
                    ...
                    if (!digitalRead(Col_IO[y]) && (old_key[x][y]) && (!slots_full)) 
                    {
                      old_key[x][y] = LOW;
                            ...
                            {  // Fn_pressed is active low so it is not pressed and normal key needs to be sent
                                  ...
                                  if (normal[x][y] == 0x60 && shiftPressed) { // Check for backtick pressed with Shift
                                    Keyboard.press(TILDE); // Directly press TILDE (~)
                                    delay(5);  // Short delay for processing
                                    Keyboard.release(TILDE); // Release TILDE (~)
                                  } 
                                  else
                                  {
                            ...
                            { // Fn is pressed so send media if a key exists in the matrix
                                if (normal[x][y] == KEY_F2) 
                                {
                                      f2Pressed = !digitalRead(Col_IO[y]);

                                } 
                                else if (normal[x][y] == KEY_F3) 
                                {
                                      f3Pressed = !digitalRead(Col_IO[y]);
                                }


                                if(media[x][y]==Consumer.VOLUME_UP || media[x][y]==Consumer.VOLUME_DOWN)
                                {
                                      for(int i=0;f2Pressed || f3Pressed;i++)
                                      {
                                        
                                      Consumer.press(media[x][y]); // media key is sent using keyboard press function per PJRC    
                                        delay(100); // delay 5 milliseconds before releasing to make sure it gets sent over USB
