...
const int MAX_NUMBERS = 25; // Change this to your desired maximum number of integers
...
strCurrentRtc.Minutes =  now.Minute();
  ...
  if (millis() - lastLoopMillis >= 1000) {  // 12000 milliseconds = 12 seconds
 ...
 int present=0;
    ...
    if(present)
    {
      ...
      // if (i + 1 == numbers[0] || i + 1 == numbers[1] || i + 1 == numbers[2] || i + 1 == numbers[3] || i + 1 == numbers[4] || numbers[5] )
       ...
       if (i + 1 == slaveIdsToReadTP[0] || i + 1 == slaveIdsToReadTP[1] || i + 1 == slaveIdsToReadTP[2])
      {
        result = 1;
        retrycounts = 3;
        do{
...
// int isInsideArray( int A[], int n, int k ){
//    for( int i = 0; i < n; i++) {
//       if( A [ i ] == k ) {
//         Serial.println("yes");
//          return 1;   
