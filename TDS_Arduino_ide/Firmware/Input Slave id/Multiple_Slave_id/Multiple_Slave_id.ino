...
int slaveIdsToReadTP[] = { 1,2,3,19,25};
...
void setup() {
  ...
  if (cardType == CARD_NONE) {
   ...
   if (Serial.available() > 0) {
    ...
    // Parse and store the integers, and get the count of stored numbers
...
strCurrentRtc.Minutes =  now.Minute();
  ...
  if (currentMinute >=  DataLogMinute) {
    ...
    for (int i = 0; i < NUM_SLAVES && isInsideArray( numbers, n, i+1 ); i++)
     {
      uint8_t result;
      float temp;
