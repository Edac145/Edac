...
void saveDataToSD(const char* data, unsigned char bLogType) {
  ...
  if(bLogType == USER_LOG)
  {
      if (!SD.exists(DATA_LOG_FILE_NAME))
      {
        ...
        if (dataFile) {
          dataFile.print("SlaveId,Date,Time,Temperature,Pressure");
          dataFile.close();
        } else {
  ...
  else if(bLogType == CALIBRATION_LOG)
  {
      if (!SD.exists(CALLIBRATION_FILE_NAME))
      {
        ...
        if (dataFile) {
          dataFile.print("SlaveId,Date,Time,Temperature,temperatureADC,Pressure,pressureADC");
          // dataFile.println(); // change
          dataFile.close();
        } else {
          log("Failed to create file");
        }
...
void setup() {
  ...
  if (!SD.exists(DATA_LOG_FILE_NAME)) {
    File dataFile = SD.open(DATA_LOG_FILE_NAME, FILE_WRITE);
    if (dataFile) {
      dataFile.println("SlaveId,Date,Time,Temperature,Pressure");
      dataFile.close();
    }
    ...
    File dataFile = SD.open(CALLIBRATION_FILE_NAME, FILE_WRITE);
    if (dataFile) {
      dataFile.println("SlaveId,Date,Time,Temperature,temperatureADC,Pressure,pressureADC");
      dataFile.close();
    }
  ...
  if (currentMinute >=  DataLogMinute) {
    ...
    for (int i = 0; i < NUM_SLAVES; i++) {
...

      {
        ...
        // receptionTime = millis(); // Record the reception time
       
        if (result == node[i].ku8MBSuccess) {
        // temp = node[i].getResponseBuffer(0);
        // press = node[i].getResponseBuffer(1);
          //  actualDelay = receptionTime - transmissionStartTime;
        ...
        RtcDateTime now = Rtc.GetDateTime();
          ...
          saveDataToSD(modbusData, CALIBRATION_LOG);
            ...
            // // Append actual delay to the SD card
            // sprintf(modbusData, "%d,%02u/%02u/%04u,%02u:%02u:%02u, %.2f, %d, %.2f, %d, %lu \n", (i + 1), now.Month(), now.Day(), now.Year(), now.Hour(), now.Minute(), now.Second(), temp, temperatureADC, press, pressureADC, actualDelay);
            // saveDataToSD(modbusData, CALIBRATION_LOG);
...

      {
        ...
        // // Record the reception time after receiving the response
        //  receptionTime = millis();
        ...
        if (result == node[i].ku8MBSuccess)
        {
          ...
          // // Append actual delay to the SD card
          // sprintf(modbusData, "%d,%02u/%02u/%04u,%02u:%02u:%02u, %.2f, %d, %lu \n", (i + 1), now.Month(), now.Day(), now.Year(), now.Hour(), now.Minute(), now.Second(), temp, temperatureADC, actualDelay);
          // saveDataToSD(modbusData, CALIBRATION_LOG);
          // //  sprintf(modbusData, "%d,%02u/%02u/%04u,%02u:%02u:%02u, %.2f, %d, %.2f, %d, %lu",(i + 1), now.Month(), now.Day(), now.Year(), now.Hour(), now.Minute(), now.Second(),temp, temperatureADC, press, pressureADC, actualDelay);
