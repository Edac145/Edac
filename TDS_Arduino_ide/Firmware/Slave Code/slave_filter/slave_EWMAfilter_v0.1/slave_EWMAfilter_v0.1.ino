...
const float alpha = 0.2;  // Adjust this value based on the desired filter strength
...
void loop() {
  ...
  // Apply EWMA filter to pressure
  pressure = alpha * pressADC + (1 - alpha) * pressure;
  ...
  // Apply EWMA filter to temperature
  temperature = alpha * tempADC + (1 - alpha) * temperature;
